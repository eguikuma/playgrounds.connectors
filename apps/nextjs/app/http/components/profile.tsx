...
export const Profile = ({ defaults }: { defaults: PokemonProfile }) => {
  const page = usePage()

  const {
    responses: { success },
  } = http.useGet(`/pokemons/${defaults.id}`, {
    key: ['pokemons', page, defaults.id],
    defaults,
  })

  const data = success?.data ?? defaults
    ...
    <Container>
      ...
      <div className="bg-white rounded-2xl border-4 border-gray-900 shadow-[12px_12px_0px_0px_rgba(0,0,0,0.8)] overflow-hidden">
        <div className="bg-linear-to-br from-red-50 via-blue-50 to-green-50 p-8 flex justify-center border-b-4 border-gray-900 relative">
          ...
          <Image
            src={data.image}
            alt={data.translation || data.name}
            width={256}
            height={256}
        ...
        <div className="p-8 bg-linear-to-b from-white to-gray-50">
          <div className="mb-6">
            <h1 className="text-4xl font-bold mb-2">{data.translation || data.name}</h1>
            <p className="text-gray-500 text-lg">No. {data.id.toString().padStart(3, '0')}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-white p-4 rounded-xl border-4 border-gray-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)]">
              ...
              <p className="text-3xl font-bold text-gray-900">{data.height / 10} m</p>
            </div>
            <div className="bg-white p-4 rounded-xl border-4 border-gray-900 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.3)]">
              <p className="text-gray-600 text-sm font-bold">重さ</p>
              <p className="text-3xl font-bold text-gray-900">{data.weight / 10} kg</p>
            </div>
          </div>
