...
export const Dashboard = () => {
  ...
  const client = {
    statuses: tanstack.sandbox.post((code: number) => `/statuses/${code}`, {}),
    redirects: {
      [Redirect.Manual]: tanstack.sandbox.post((code: number) => `/redirects/${code}`, {
        options: { redirect: Redirect.Manual },
      }),
      [Redirect.Follow]: tanstack.sandbox.post((code: number) => `/redirects/${code}`, {
        options: { redirect: Redirect.Follow },
      }),
      [Redirect.Error]: tanstack.sandbox.post((code: number) => `/redirects/${code}`, {
  ...
  return (
    <Container>
      ...
      <div className="space-y-8">
        ...
        <RequestInterceptor {...(snapshot ? snapshot.request : { before: {}, after: {} })} />

        <ResponseInterceptor {...(snapshot ? snapshot.response : { status: null })} />
