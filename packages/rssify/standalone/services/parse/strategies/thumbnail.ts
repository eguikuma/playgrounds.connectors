...
const CodePointLimits = {
  FirstPrintable: 32,
  Delete: 127,
  MaxUnicode: 0x10ffff,
} as const

const ImageTagLimits = {
  MaxAttributeLength: 1999,
} as const

const field = (value: Fields['mediaContent'] | undefined): string | undefined => {
  if (!value) {
    return undefined
  }
...
const html = (text: string): string =>
  text
    .replace(/&#(\d+);/g, (_, code) => {
      ...
      if (
        point < CodePointLimits.FirstPrintable ||
        point === CodePointLimits.Delete ||
        point > CodePointLimits.MaxUnicode
      ) {
        return ''
      }
...
const image = (text: string, base?: string): string | undefined => {
  const pattern = new RegExp(
    `<img(?:\\s[^>]{0,${ImageTagLimits.MaxAttributeLength}})?\\ssrc\\s*=\\s*["']([^"']+)["']`,
    'i',
  )
